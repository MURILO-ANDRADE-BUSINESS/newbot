...
let AuthUserUseCase = class AuthUserUseCase {
    ...
    execute({ email, password }) {
        return __awaiter(this, void 0, void 0, function* () {
            const user = (yield this.userRepository.findByEmail({
                email,
            }));
            if (!user) {
                throw new AppError_1.AppError('Email incorreto!', 401);
            }
            const passwordMatch = yield (0, bcrypt_1.compare)(password, user.password);
            if (!passwordMatch) {
                throw new AppError_1.AppError('Senha incorreta!', 401);
            }
            const token = (0, jsonwebtoken_1.sign)({ id: user.id }, '702bfcb73eac4edf9edc10229ae774cd', {
                expiresIn: '1d',
            });
            const tokenReturn = {
                user,
